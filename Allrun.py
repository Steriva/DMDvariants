...
for dataset in ['PDE_KS']:
    ...
    for ii, fold in enumerate(tune_folders):
        ...
        last_run = os.listdir(f'../../results/tune_results/{dataset}/DMD_{method}/{fold}/')[-1]
        os.makedirs(f'config/{method}/', exist_ok=True)
        yaml_file = sorted(os.listdir(f'../../results/tune_results/{dataset}/DMD_{method}/{fold}/{last_run}/'))[0]
        os.system(f'cp ../../results/tune_results/{dataset}/DMD_{method}/{fold}/{last_run}/{yaml_file} config/{method}/.')
