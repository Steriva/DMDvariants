...
More in detail, let $\vec{x}(t)\inR^{\mathcal{N}_h}$ being some high-dimensional measurements of the state of the system over time $t$, collected at different time instants and arranged in a snapshot matrix of size $\mathcal{N}_h \times N_t$ in the following way:
```math
